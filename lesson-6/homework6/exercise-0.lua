...
-- Implement if else ifelse while and logical binary operators
-- Note we use if2 in the ast tag (but not in the pug language) rather than if because we use metaprogramming which involves running the code via lua interpreter and if will
...
reserved_words = {"return", "if", "else", "elseif", "while", "and", "or"}
...
local grammar = lpeg.P{"prog",
  ...
  stat = (lvalue * T"=" * exp) / nodeStat + (ret * exp / node("return", "exp")) + (T"@" * exp / node("print", "exp")) + elseifstat + whilestat,
