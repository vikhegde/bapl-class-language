...
-- Implement bounds checking for arrays:  getindex, setindex, newarray
...
local grammar = lpeg.P{"prog",
  ...
  rvalue = ((lpeg.C(ID) * T"[" * exp * T"]" / node("getindex", "array", "index")) + (ID / node("rvar", "rvar"))) * space, 
  lvalue = ((lpeg.C(ID) * T"[" * exp * T"]" / node("setindex", "array", "index")) + (ID / node("lvar", "lvar"))) * space,
  newExp = RESV"new" * space * T"[" * exp * T"]" / node("newarray", "size") * space,
...
function Compiler:codeStat(ast)
	if ast.tag == "assign" then
		if ast.lhs.tag == "lvar" or ast.lhs.tag == "newarray" then
...
local function run(code, mem, stack)
	...
	while true do
		...
		elseif code[pc] == "load" then
			top = top + 1
			if type(mem[code[pc+1]]) == "table" then
				stack[top] = pt.pt(mem[code[pc+1]])
			else
		...
		elseif code[pc] == "getindex" then
			print("pc=", pc, code[pc], code[pc+1])
			if stack[top] <= 0 or stack[top] > mem[code[pc+1]]["size"] then
				error("Runtime error: array access beyond bounds, array name = "..code[pc+1]) 
				os.exit()
			end
			stack[top] = mem[code[pc+1]][stack[top]]
			pc = pc + 2
		elseif code[pc] == "setindex" then
			print("pc=", pc, code[pc], code[pc+1], stack[top], stack[top - 1])
                        print(pt.pt(mem[code[pc+1]]["size"]))
			if stack[top] <= 0 then
				error("Runtime error: array access beyond bounds, array name = ", mem[code[pc+1]]) 
				os.exit()
                        elseif stack[top] > mem[code[pc+1]]["size"] then
                        	mem[code[pc+1]]["size"] = stack[top]
			end
			mem[code[pc+1]][stack[top]] = stack[top-1]
			pc = pc + 2
			top = top - 2
