...
reserved_words = {"return", "if2", "else2", "elseif2"}
...
local grammar = lpeg.P{"prog",
  ...
  block = T"{" * stats * T"}" + (T"{" * T"}"),
  elseifstat = ((RESV"elseif2" + RESV"if2") * exp * block * (elseifstat)^-1 * (RESV"else2" * block)^-1 / node("if2", "ifcond2", "thenBlock", "elseBlock")),
  stat = (lvalue * T"=" * exp) / nodeStat + (ret * exp / node("return", "exp")) + (T"@" * exp / node("print", "exp")) + elseifstat
  		, 
