...
-- local variables, local variables in blocks iand functions
...
local grammar = lpeg.P{"prog",
  ...
  stat = (RESV"var" * lpeg.C(ID) * space * T"=" * exp / node("local", "name", "init")) + (lvalue * T"=" * (newExp + exp)) / nodeStat + funcCall + (ret * exp / node("return", "exp")) + (T"@" * exp / node("print", "exp")) + elseifstat + whilestat,
...
Compiler = { funcNames = {},  vars = {}, nvars = 0, locals = {}}
