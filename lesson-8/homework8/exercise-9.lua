...
-- implement code generation for function parameters and function arguments
...
local grammar = lpeg.P{"prog",
  ...
  funcParams = lpeg.Ct((lpeg.C(ID) * (T"," * lpeg.C(ID))^0)^-1),
...
function Compiler:codeFuncCall(ast)
	...
	if #(self.funcNames[ast.name].params) ~= #(ast.args) then
...
function Compiler:codeFunc(ast)
	...
	-- to support forward declaration of functions i.e. function prototypes
		...
		self.funcNames[ast.name] = {code = code, params = ast.params}
		return
	end
	if self.funcNames[ast.name] then
		if #(self.funcNames[ast.name].params) ~= #(ast.params) then
			error("Function prototype number of arguments does not match function definition for function: " .. ast.name)
		end
...
local function compile(compiler, ast)
	for i = 1,#ast do
		...
		local hashTable = {}
		for j = 1,#(ast[i].params) do
			if hashTable[ast[i].params[j]] then
				error("Function parameter " .. ast[i].params[j] .. " redeclared.")
			else
				hashTable[ast[i].params[j]] = 1
			end
		end
	end
