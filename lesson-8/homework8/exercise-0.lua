...
-- carry over from Week-7. blocks in AST and block support in compiler but no blocks in code generation
...
reserved_words = {"return", "if", "else", "elseif", "while", "new", "function"}
...
local grammar = lpeg.P{"prog",
  ...
  funcDef = RESV"function" * space * lpeg.C(ID) * space *  T"(" * T")" * space * (block + T";") * space / node("funcDef", "name", "stats"),
  funcCall = lpeg.C(ID) * T"(" * T")" * space / node("funcCall", "name"),
  ...
  ID = (alpha + id_special) * lpeg.P(function (s,p) start_id = p-1; return true end) * (alpha + id_special + num)^0 * lpeg.P(function (s, p) stop_id = p-1; string.sub(s, start_id, stop_id); if exclusion_list:match(string.sub(s, start_id, stop_id)) then print(string.sub(s, start_id, stop_id)); return false else return true end end), 
  ...
  stat = (lvalue * T"=" * (newExp + exp)) / nodeStat + funcCall + (ret * exp / node("return", "exp")) + (T"@" * exp / node("print", "exp")) + elseifstat + whilestat,
...
Compiler = { funcNames = {},  vars = {}, nvars = 0}
