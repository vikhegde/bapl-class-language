...
-- implement default argument for the last argument only
...
local grammar = lpeg.P{"prog",
  ...
  funcParams = lpeg.Ct((lpeg.C(ID) * (T"," * lpeg.C(ID))^0 * space * (T"=" * exp)^-1)^-1),
  funcCall = lpeg.C(ID) * T"(" * funcArgs * T")" * space / node("funcCall", "name", "args"),
  funcArgs = lpeg.Ct((exp * (T"," * exp)^0)^-1),
...
function Compiler:codeFuncCall(ast)
	local paramCount = #(self.funcNames[ast.name].params)
	local argCount = #(ast.args) 
	local defaultArg = self.funcNames[ast.name].default
	if  paramCount ~= argCount then
		if (paramCount == argCount + 1) and defaultArg then
			ast.args[#(ast.args) + 1] = defaultArg
		else
			error("Number of function parameters does not match arguments in function call for function: " .. ast.name)
		end
	end
	for i = 1,#(ast.args) do
	...
	self:addCode("call")
	self:addCode(self.funcNames[ast.name].code)
end
...
function Compiler:findIfLocal(name)
	...
	-- if local variable is not a true local, it could be a function parameter
	local params = self.params
	for i = 1,#params do
		if params[i] == name then
			-- base points to the last paramter so if i == #params, then we return -0 with is added to base
			return -(#params - i)
		end
	end
	return nil
...
function Compiler:codeFunc(ast)
	...
	-- Check if last parameter has default value
	if #(ast.params) > 0 then
		local lastParam = ast.params[#(ast.params)]
		local paramType = type(lastParam) 
		if paramType == "string" then
			;
		elseif paramType == "table" then
			ast.default = lastParam
			table.remove(ast.params)
		else
			error("invalid default argument for last parameter in function: " .. ast.name)
		end
	end

	local hashTable = {}
	for j = 1,#(ast.params) do
		if hashTable[ast.params[j]] then
			error("Function parameter " .. ast.params[j] .. " redeclared.")
		else
			hashTable[ast.params[j]] = 1
		end
	end

	-- to support forward declaration of functions i.e. function prototypes
        if not ast.stats then
		self.funcNames[ast.name] = {code = code, params = ast.params, default = ast.default}
		return
	end
	if self.funcNames[ast.name] then
		if #(self.funcNames[ast.name].params) ~= #(ast.params) then
			error("Function prototype number of params does not match function definition for function: " .. ast.name)
		end
	...
	self:addCode(0)
	self:addCode("ret")
	-- the code[pc+1] for ret tells us how many to pop. We want to pop locals and function arguments
	self:addCode(#self.locals + #self.params)
end
...
local function compile(compiler, ast)
	...
	elseif #(main.params) ~= 0 then
		error("main() function cannot take any parameters")
	end
	return main.code
end
