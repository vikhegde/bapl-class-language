...
-- Implement single main function
...
local grammar = lpeg.P{"prog",
  prog = space * (funcDef)^1 * space * -1,
  funcDef = RESV"function" * space * lpeg.C(ID) * space *  T"(" * T")" * space * block * space / node("function", "name", "stats"),
...
local function compile(compiler, ast)
	compiler:codeFunc(ast)
	local main = compiler.funcNames["main"]
	if not main then
		error("Function main has not been declared and is mandatory")
	end
	return compiler.code -- Note this is main code
end
